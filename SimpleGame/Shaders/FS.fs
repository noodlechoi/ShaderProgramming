...
vec4 Flag()
{
    ...
    float sinValue = v_TPos.x * amp * (((sin(sinInput) + 1) / 2) - .5) + 0.5;  // 0 -> 0.5
...
void Flame()
{
    ...
    float fWidth = 0.0;
    float width = 0.5 * mix(1, fWidth, newY);   // 연기 => mix(fWidth, 1, newY);
    float grey = 0;
...
void TextureSampling()
{
    ...
    float offsetX = 0.01;
    c[0] = texture(u_RGBTex, vec2(v_TPos.x - offsetX * 2, v_TPos.y));
    c[1] = texture(u_RGBTex, vec2(v_TPos.x - offsetX * 1, v_TPos.y));
    c[2] = texture(u_RGBTex, vec2(v_TPos.x - offsetX * 0, v_TPos.y));
    c[3] = texture(u_RGBTex, vec2(v_TPos.x + offsetX * 1, v_TPos.y));
    c[4] = texture(u_RGBTex, vec2(v_TPos.x + offsetX * 2, v_TPos.y));
...
void Nums()
{
    ...
    float tx = v_TPos.x / col;
    float ty = v_TPos.y / row;
    float offsetX = fract(index/col);
    float offsetY = floor(index / col)/row;
    vec2 newTex = vec2(tx + offsetX, ty + offsetY);
